...
public class MedicineRestControllerImpl implements MedicineRestController {
    ...
    @Override
    public ResponseEntity<List<SimpleMedicineDto>> getAllMedicines() {
        List<Medicine> allMedicines = medicineService.getAllMedicines();

        List<SimpleMedicineDto> simpleMedicineDtos = dtoMapper.convertToSimpleMedicineDto(allMedicines);

        return new ResponseEntity<>(simpleMedicineDtos, HttpStatus.OK);
    }
